<template>
  <svg 
    version="1.1"
    xmlns="http://www.w3.org/2000/svg"
    xmlns:xlink="http://www.w3.org/1999/xlink"
    x="0px" y="0px"
    viewBox="0 0 512.002 512.002"
  >
    <path 
      d="
        M350.755,246.298L311.331,25.489c-0.045-0.252-0.101-0.463-0.157-0.634l-41.888,25.421
        c-2.069,1.256-4.093,5.44-3.503,8.751c0.074,0.41,0.11,0.825,0.109,1.242l-0.09,46.044L350.755,246.298z
      "
    />
    <path 
      d="
        M265.369,266.302l-131.046-100.96l-33.671,20.436c0.125,0.127,0.285,0.273,0.488,0.429l211.518,162.958l1.154-3.041
        L265.369,266.302z
      "
    />
    <path 
      d="
        M171.975,112.404L78.879,40.935c-2.674-2.062-7.318-2.199-9.386-0.941L27.605,65.412c0.126,0.129,0.287,0.274,0.491,0.432
        l217.148,167.294L171.975,112.404z
      "
    />
    <path 
      d="
        M476.569,430.273c-3.406-5.619-7.762-10.512-12.946-14.546c-0.088-0.069-0.172-0.138-0.256-0.21
        c-4.368-3.381-47.086-36.87-68.433-69.465c-0.043-0.067-0.088-0.134-0.129-0.203l-40.56-66.834l-24.935,65.69l18.135,29.885
        c0.041,0.069,0.083,0.138,0.122,0.207c19.064,33.995,29.05,87.372,30.028,92.78c0.024,0.106,0.046,0.213,0.066,0.32
        c1.183,6.455,3.514,12.578,6.924,18.198c7.456,12.284,19.249,20.931,33.207,24.346c13.957,3.412,28.409,1.189,40.695-6.265
        c12.285-7.456,20.933-19.249,24.347-33.207C486.249,457.01,484.025,442.557,476.569,430.273z
      "
    />
    <path 
      d="
        M185.841,1.513c-0.899-1.481-2.837-1.958-4.321-1.056l-41.983,25.479c-1.481,0.899-1.954,2.837-1.056,4.317
        l181.628,299.285l24.935-65.69L185.841,1.513z
      "
    />
  </svg>
</template>

<script>
export default {
  name: 'IconLeek',
}
</script>

<style scoped>
svg path:nth-child(1) {
  fill: #2A402B;
}

svg path:nth-child(2) {
  fill: #7AA868;
}

svg path:nth-child(3) {
  fill: #D0D998;
}

svg path:nth-child(4) {
  fill: #E3E0DB;
}

svg path:nth-child(5) {
  fill: #7AA868;
}
</style>
